# WebDX — Architecture & Internals

> This is the deep technical reference for contributors: how each feature is wired,
> the extension points used, the hard-won gotchas, and how to build, install, debug
> and test. For the user-facing overview see the [README](../README.md).

All features resolve everything from the actual source files on disk (`.module.scss`,
locale JSON, `StyleSheet.create` objects, `index.ts` barrels), so they keep working
**independently of the TypeScript language service** — important because this project
runs on the experimental **TypeScript-Go (tsgo)** engine, which does not load TS
service plugins like `typescript-plugin-css-modules`.

The plugin id stays `com.webdx.css-modules-scoped-usages` (kept for update
continuity); only the display name is **WebDX**. Built and verified against
**WebStorm 2026.1 (build 261)**.

---

## Features (with handler references)
...
13. **Scoped Find Usages on a style key.** Lists only the real references (`<binding>.<key>`
    accesses and `const { key } = styles` destructuring) in the
...
18a. **"Where does this re-export go directly?" on a re-export (Cmd+Click / Find Usages).** Cmd+Click
    (or Alt+F7) on the name in an `export { X } from '…'` opens the standard **Show Usages** popup
    listing the **direct, single-hop** sites that draw `X` from *this* barrel file: the next-level
    re-export that forwards it (`export { X } from './thisBarrel'`) and any module importing it
    straight from here (`import { X } from '@/this/barrel'`). It is deliberately **not transitive** —
    it does NOT chase `X` through further barrels to the leaf components that ultimately render it, so
    from `UI-KIT/UniversalCard/index.ts` the answer is just `UI-KIT/index.tsx` (which re-exports it
    onward), not the screens that import from `@/UI-KIT`. The element shown is the referencing
    import/export site itself — no recursion, no expansion into the consumer's local uses. Reuses
    `DeadReExports.classify`/`consumedNames` (so `@/`-alias / `require` handling is identical) over a
    single `ReferencesSearch` on the barrel file. Cmd+Click is intercepted in the shared
    `GotoDeclaration` action override (the only reliable hook under the TS-Go fork — see gotcha #7);
    `export *` (no name to click) and a local `export { X }` (no `from`) are out of scope.

    **Cmd+Click UX.** The override builds its own chooser popup (not the Show Usages action, which
    sorts and auto-jumps on a single result): the FIRST row navigates into the component itself —
    the source declaration the re-export points at, resolved via
    `BarrelReExportUsages.resolveSource` (`ES6PsiUtil.getFromClauseResolvedReferences` +
    `resolveSymbolInModules`) — and the remaining rows are the direct sites from the collector. It is
    always a popup, even for one entry, so go-to-component and see-who-imports-it share one gesture.
    Alt+F7 still goes through the find-usages handler (no synthetic component row there).
    → `BarrelReExportUsages` (detector + direct collector + `resolveSource`),
    `BarrelReExportFindUsagesHandlerFactory`, `CssModuleGotoDeclarationAction` (popup)
...
A WebDX tool window in the left stripe with one button per check (unused CSS-module
classes, unused SCSS symbols, unused/unknown RN keys, dead exports/re-exports, unknown
CSS classes, i18n checks…) plus **Run all** and a **Stop** button. Each button runs only
its inspection(s) across the whole project in one pass; results open in the standard
Inspection Results window. Runs through the platform's own Inspect-Code pipeline, so the
scan is fully cancellable.
