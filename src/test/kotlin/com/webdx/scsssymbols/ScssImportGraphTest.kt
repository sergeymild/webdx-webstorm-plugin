...
class ScssImportGraphTest : BasePlatformTestCase() {
    ...
    fun testProvidedFollowsForwardAndImportTransitively() {
        myFixture.addFileToProject("c.scss", "\$c: 1;")
        myFixture.addFileToProject("b.scss", "@forward './c.scss';\n\$b: 1;")
        val a = myFixture.addFileToProject("a.scss", "@forward './b.scss';\n\$a: 1;")
        // a real, unrelated file that `a` neither forwards nor imports
        val unrelated = myFixture.addFileToProject("unrelated.scss", "\$u: 1;")
        val provided = ScssImportGraph.provided(project, a.virtualFile)
        assertEquals(
            setOf("a.scss", "b.scss", "c.scss"),
            provided.map { it.name }.toSet(),
        )
        // unrelated existing file is not pulled in
        assertFalse(provided.contains(unrelated.virtualFile))
    }
